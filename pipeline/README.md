...
Atomic species are also embedded (generated by
`molecular/embedded/gen_atomic_dat.py`): **HI** (Lya/Lyb/Lyg,
Ha/Hb/Hg), **HeII** (same series, Z = 2), and **HeI** (584/537/10832/
3889 A). HI/HeII A-values are computed exactly from hydrogenic
wavefunctions, so Lyman alpha reproduces the textbook
`sigma_0 = 5.9e-14 (T/1e4)^-1/2 cm^2`:

```python
ti = TransitionInfo( 'HI', value = 1215.67, unit = 'angstrom' )  # Lya
```
