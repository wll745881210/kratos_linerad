...
**Table sizes and GPU memory:** ph_mode 2 moves the USampler CDF (251×40 = 10,040 floats = 39.2 KiB) and a new 1D Voigt table (5,000 floats = 19.5 KiB) from global memory into constant memory. The 2D Voigt table (64×512 = 128 KiB, used in ph_mode 0/1) is removed from the device entirely in ph_mode 2 (kept host-side only for sampling the 1D table). The R_IIA kernel table (200×200×40 = 1,600,000 floats = 6.1 MiB) is too large for constant memory (~157× the 64 KiB HW limit) and stays in global memory in all modes. Total constant-memory usage in ph_mode 2: 39.2 + 0.16 + 19.5 = 59.8 KiB (of 64 KiB HW limit). The `free_dev_mem` flag (set at `init()`: `true` for ph1, `false` for ph2) controls global vs constant allocation. `ph_mode=2`/`3` use the constant-memory pool — **not freed** in `finalize` (`free_dev_mem=false`); the 60 KiB const pool is a bump allocator.
