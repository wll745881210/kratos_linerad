...
**Imaging (two-step, runs on the FINAL cycle only):** the `[imaging]` section controls the camera + channel grid. When `enabled=0` (default) the s_cam field is neither allocated nor initialised — non-imaging runs are unaffected. The imaging uses the full **R_IIA kernel** (precomputed 3-D table, `riia_table.h:riia_table_t::build`) for the scattering source function toward the camera, correctly capturing the angle–frequency correlation (see `docs/PHYSICS.md` §12.2, §12.6).
...
- `2` – R_IIA: same USampler kernel, but tables in **constant memory**: log-CDF USampler (251×40, `du=0.048`) + 1D log-space Voigt table (5000 pts, `u∈[0,50]`, built from the host-side scipy 2D table). Fastest exact mode.
...
All R_IIA modes (1/2/3) share the same scattering kernel (`g = dir_old·dir` directional correlation). The USampler conditional is `P(u|x) ∝ exp(−u²)/(a²+(x−u)²)` (`n_u=251`, `u∈[−6,6]`, `du=0.048`, `n_xg=40`, `xg∈[0,300]` mixed linear+log). The R_IIA kernel density `R(Δ; x_in, g) = Σ_k pdf[k]·Gauss(y_k; σ=sin_g/√2)` is precomputed as a 200×200×40 table (`Δ=x_out−x_in∈[−10,10]`, `|x_in|∈[0,120]`, `g∈[−1,1]`, with analytic asymptotic for `|x_in|≥120`), used in imaging source-function accumulation (see `docs/PHYSICS.md` §12.2 for the full definition). Convention B (gas-frame `x_in`, center `u_k(g−1)`) is used — conserves outgoing frequency `x_out`. The code variable is named `x_pp` but represents `x_in` (gas-frame incoming frequency). Modes 1 and 2 agree to ~1–2% (`med|x|`); use `2` for production, `1` for debug (global-mem table).

**Table sizes and GPU memory:** Both the USampler CDF (251×40 = 10,040 floats = 39.2 KiB) and the R_IIA kernel table (200×200×40 = 1,600,000 floats = 6.4 MiB) are constructed on the **GPU** using `float_t` (single precision) via `build_usampler_gpu_kernel` (40 threads) and `build_riia_gpu_kernel` (32,000 blocks × 64 threads), respectively — replacing the previous ~2.5 s CPU construction with ~0.05 s of GPU work (50× speed-up). After construction, if `use_const_mem` (ph_mode 2/3), the USampler CDF + xg are copied to **constant memory** (Option B, 39.4 KiB) for broadcast-cache reads in the MCRT hot path. The R_IIA table (6.4 MiB, too large for the 64 KiB HW limit) stays in global memory in all modes. The 2D Voigt table (64×512 = 128 KiB, used in ph_mode 0/1) is removed from the device entirely in ph_mode 2 (kept host-side only for sampling the 1D table). Total constant-memory usage in ph_mode 2: 39.2 + 0.16 + 19.5 = 59.8 KiB (of 64 KiB HW limit). The `free_dev_mem` flag (set at `init()`: `true` for ph1, `false` for ph2) controls global vs constant allocation. `ph_mode=2`/`3` use the constant-memory pool — **not freed** in `finalize` (`free_dev_mem=false`); the 60 KiB const pool is a bump allocator.
...
| `usr_ext/line_rt/intg.h` | Integrator parameters, camera (`dir_cam`, `q_cam`), channel grid (`d_v_chan`), Voigt interpolation table, `riia_table_t riia` (USampler + R_IIA kernel table, GPU-constructed) |
| `usr_ext/line_rt/riia_table.h` | Standalone `riia_table_t` struct: USampler CDF (GPU `build_usampler_gpu_kernel`, 40 threads) + R_IIA kernel table (GPU `build_riia_gpu_kernel`, 32k blocks × 64 threads), const-mem copy (Option B), device lookup (`lookup`, `sample_upar`) |
