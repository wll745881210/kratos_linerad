...
Physics replicated exactly:
  ...
  - Excitation: flx * I(v, b, sv) via overlap integral
    I = exp(-dv^2/s2) * b/sqrt(s2)
  - proc_phys at cell entry (flux deposition),
    proc_geo for transport/scattering
...
import numpy as np;   # numba njit globals require the full module binding

from numba import njit, prange;

############################################################
#  Parallel support (thread id + capability flag)
...
# "Unlimited" scatter cap — must exceed Neufeld
# N_scat ~ 1.6*tau0*(a*tau0)^(1/3)
...
def build_usampler( a_voigt, u_max = 6.0, du = 5e-3, n_lin = 101, \
                    n_log = 121, x_max = 300.0 ):
    cache_key = ( a_voigt, du, n_lin, n_log, x_max );
    if cache_key in _usampler_cache:
        return _usampler_cache[ cache_key ];

    u_grid = np.arange( -u_max, u_max + du, du, dtype = np.float32 );
    x_lin  = np.linspace( 0.0, 8.0, n_lin, dtype = np.float32 );
    x_log  = np.logspace( np.log10( 8.0 ), np.log10( x_max ), n_log, \
                          dtype = np.float32 )[ 1 : ];
    xg_grid = np.concatenate( [ x_lin, x_log ] );

    G = np.exp( -u_grid ** 2 );
    D = u_grid[ np.newaxis, : ] - xg_grid[ :, np.newaxis ];
    W = G[ np.newaxis, : ] / ( np.float32( a_voigt ) ** 2 + D ** 2 );
...
def _cdf_lookup_row( C_grid, ix, u_vec, rval ):
    ...
    return u_vec[ k - 1 ] + ( rval - row[ k - 1 ] ) / denom \
           * ( u_vec[ k ] - u_vec[ k - 1 ] );
...
def _scatter_riia_usampler( x_freq, u_grid, xg_grid, C_grid ):
    ...
    if x_freq < 0.0:
        sgn = np.float32( -1.0 );
    u_par = sgn * _sample_u_par_usampler( np.float32( abs( x_freq ) ), \
                                          u_grid, xg_grid, C_grid );
    x_at  = np.float32( x_freq - u_par );
    g     = np.float32( 2.0 * np.float32( np.random.random( ) ) - 1.0 );
    u_perp = np.float32( np.sqrt( -2.0 * np.log( \
                  np.float32( np.random.random( ) ) + 1e-35 ) ) \
                  * np.cos( 2.0 * np.pi * np.float32( np.random.random( ) ) ) \
                  / SQRT2 );
    u_par_n = np.float32( g * u_par \
               + np.sqrt( np.float32( 1.0 - g * g ) ) * u_perp );
    return np.float32( x_at + u_par_n );
...
def _transport_photon( x, d0, pr, vl, sv, ns_in, n_left,
    ...
    tr     = np.float32( -np.log( 1e-4 + 0.9999 * \
    ...
    while n_left > 0 and pr > 1e-30:
        ...
        exc_out[ dep_off + ci ] += fc * np.exp( -vl * vl / s2 ) \
                                   * b_cell / np.sqrt( s2 );
        ...
        if dtau_s > tr and ns_max > 0:
            ...
            mu   = np.float32( 2.0 * \
                    np.float32( np.random.random( ) ) - 1.0 );
            smu  = np.float32( np.sqrt( np.float32( 1.0 - mu * mu ) ) );
            ph_ang = np.float32( 2.0 * np.pi * \
                      np.float32( np.random.random( ) ) );
            d0 = np.float32( smu * np.cos( ph_ang ) );

            if ph_mode == 1:
                x_freq = vl / ( b_cell + 1e-35 );
                vl = _scatter_riia_usampler( x_freq, u_grid, xg_grid, \
                                             C_grid ) * b_cell;
                sv = b_cell / SQRT2;
            else:
                vl = 0.0;
                ...
                vl += np.float32( np.sqrt( -2.0 * np.log( \
                      u1 + 1e-35 ) ) * np.cos( 2.0 * np.pi * u2 ) * \
                      b_cell / SQRT2 );
                sv = b_cell / SQRT2;
...
def voigt_H( a, u ):
    ...
    if s >= 15.0:
        ...
        w = t * ( 1.410474 + t * t * 0.5641896 ) / \
            ( 0.75 + t * t * ( 3.0 + t * t ) );
    elif y >= 0.195 * x - 0.176:
        w = ( ( 16.4955 + t * ( 20.20933 + t * ( 11.96482 \
              + t * ( 3.778987 + t * 0.5642236 ) ) ) ) / \
              ( 16.4955 + t * ( 38.82363 + t * ( 39.27121 \
              + t * ( 21.69274 + t * ( 6.699398 + t ) ) ) ) ) );
    else:
        w = ( np.exp( t * t ) - t * ( 36183.31 - t * t * ( 3321.9905 \
              - t * t * ( 1540.787 - t * t * ( 219.0313 - t * t \
              * ( 35.76683 - t * t * \
                ( 1.320522 - t * t * 0.56419 ) ) ) ) ) ) / \
              ( 32066.6 - t * t * ( 24322.84 - t * t * ( 9022.228 - t * t \
              * ( 2186.181 - t * t * ( 364.2191 - t * t * ( 61.57037 - t * t \
              * ( 1.841439 - t * t ) ) ) ) ) ) ) );
    return w.real;
...
def run_mcrt( mesh, photons, b_sca, mfp_i_sca_0, mfp_i_abs_0,
    ...
    b_arr = np.full( n_tot, np.float32( b_sca ) );
    ...
    mfp_s0_arr = np.full( n_tot, np.float32( mfp_i_sca_0 ) );
    mfp_a0_arr = np.full( n_tot, np.float32( mfp_i_abs_0 ) );

    ph = np.asarray( photons, dtype = np.float32 ).copy( );
    n_ph  = ph.shape[ 0 ];
    n_col = ph.shape[ 1 ];

    if n_col < 9:
        ph = np.pad( ph, ( ( 0, 0 ), ( 0, 9 - n_col ) ), \
                     constant_values = 0.0 );

    esc_vel   = np.zeros( n_ph, dtype = np.float32 );
    esc_pr    = np.zeros( n_ph, dtype = np.float32 );
    esc_sv    = np.zeros( n_ph, dtype = np.float32 );
    ...
    seeds = ( int( seed ) * 1000003 + np.arange( n_ph, \
             dtype = np.int64 ) ) % ( 2**31 - 1 );
    ...
    if ph_mode == 1 and a_voigt > 1e-9:
        ...
        u_grid  = np.zeros( 1, dtype = np.float32 );
        xg_grid = np.zeros( 1, dtype = np.float32 );
        C_grid  = np.zeros( ( 1, 1 ), dtype = np.float32 );

    if use_parallel:
        n_thr = multiprocessing.cpu_count( );
        flx_priv = np.zeros( n_thr * n_tot, dtype = np.float32 );
        exc_priv = np.zeros( n_thr * n_tot, dtype = np.float32 );
        _transport_parallel(
            ph[ :, 0 ].copy( ), ph[ :, 3 ].copy( ),
        ...
        flx_out = np.zeros( n_tot, dtype = np.float32 );
        exc_out = np.zeros( n_tot, dtype = np.float32 );
        _transport_serial(
            ph[ :, 0 ].copy( ), ph[ :, 3 ].copy( ),
            ph[ :, 6 ].copy( ), ph[ :, 7 ].copy( ), ph[ :, 8 ].copy( ),
            n_step_arr, n_scat_arr, seeds,
    ...
    mask = esc_flag == 1;
    escaped = np.column_stack( [ esc_vel[ mask ], esc_pr[ mask ], \
                                 esc_sv[ mask ] ] ) if mask.any( ) \
              else np.zeros( ( 0, 3 ), dtype = np.float32 );

    # Termination reasons: 1=escaped, 2=step budget, 3=proper decayed,
    # 4=degenerate dl
    return { 'flx' : flx_out, 'excitation_flux' : exc_out, \
             'escaped' : escaped, 'term_reason' : esc_flag, \
             'n_scat' : nscat_out };
...
def mcrt_slab( *, n_cell = 64, L_slab = 1.49598e13,
               ...
               a_voigt = 0.0, seed = 42, source = 'midplane', \
               parallel = True ):
    """
    ...
    mesh = {
        ...
        'x_min'  : np.array( [ -L_slab / 2, 0.0, 0.0 ], dtype = np.float32 ),
        'dx'     : np.array( [ dx_val, dy_val, dz_val ], dtype = np.float32 ),
    };
    ...
    ph = np.zeros( ( n_photons, 9 ), dtype = np.float32 );
    if source == 'midplane':
        ph[ :, 0 ] = 0.0;
