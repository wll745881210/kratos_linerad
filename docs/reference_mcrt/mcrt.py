...
def _scatter_riia_usampler( x_freq, u_grid, xg_grid, C_grid ):
    ...
    if x_freq < 0.0:
        sgn = -1.0;
    u_par = sgn * _sample_u_par_usampler( abs( x_freq ), u_grid, \
                                          xg_grid, C_grid );
    x_at  = x_freq - u_par;
    g     = 2.0 * np.random.random( ) - 1.0;
    u_perp = np.sqrt( -2.0 * np.log( np.random.random( ) + 1e-35 ) ) \
             * np.cos( 2.0 * np.pi * np.random.random( ) ) / SQRT2;
    ...
    while n_left > 0 and pr > 1e-30:
        ...
        if dtau_s > tr and ns_max > 0:
            ...
            if ph_mode == 1:
                ...
                u1 = np.random.random( );
                u2 = np.random.random( );
                vl += np.sqrt( -2.0 * np.log( u1 + 1e-35 ) ) \
                      * np.cos( 2.0 * np.pi * u2 ) * b_cell / SQRT2;
                sv = b_cell / SQRT2;
