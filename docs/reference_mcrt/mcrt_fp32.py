...
def build_usampler(a_voigt, u_max=6.0, du=5e-3, n_lin=101, n_log=121, x_max=300.0):
    ...
    x_log = np.logspace(np.log10(8.0), np.log10(x_max), n_log, dtype=np.float32)[1:]
...
def _sample_u_par_usampler(xa, u_grid, xg_grid, C_grid):
    ...
    if j >= n_xg - 1:
        j = n_xg - 2
    f = np.float32((xa - xg_grid[j]) / (xg_grid[j + 1] - xg_grid[j]))
...
def _scatter_riia_usampler(x_freq, u_grid, xg_grid, C_grid):
    ...
    if x_freq < 0.0:
        sgn = np.float32(-1.0)
    u_par = sgn * _sample_u_par_usampler(np.float32(abs(x_freq)), u_grid, xg_grid, C_grid)
    x_at = np.float32(x_freq - u_par)
    g = np.float32(2.0 * np.float32(np.random.random()) - 1.0)
    u_perp = np.float32(np.sqrt(-2.0 * np.log(np.float32(np.random.random()) + 1e-35)) * np.cos(2.0 * np.pi * np.float32(np.random.random())) / SQRT2)
    u_par_n = np.float32(g * u_par + np.sqrt(np.float32(1.0 - g * g)) * u_perp)
...
def _transport_photon(x, d0, pr, vl, sv, ns_in, n_left,
    ...
    tr = np.float32(-np.log(1e-4 + 0.9999 * np.float32(np.random.random())))
    reason = 2
    n_scat = 0
    ...
    while n_left > 0 and pr > 1e-30:
        ...
        if dtau_s > tr and ns_max > 0:
            ...
            mu = np.float32(2.0 * np.float32(np.random.random()) - 1.0)
            smu = np.float32(np.sqrt(np.float32(1.0 - mu * mu)))
            ph_ang = np.float32(2.0 * np.pi * np.float32(np.random.random()))
            ...
            if ph_mode == 1:
                ...
                vl += np.float32(np.sqrt(-2.0 * np.log(u1 + 1e-35)) * np.cos(2.0 * np.pi * u2) * b_cell / SQRT2)
                sv = b_cell / SQRT2
...
def run_mcrt(mesh, photons, b_sca, mfp_i_sca_0, mfp_i_abs_0,
    ...
    ]) if mask.any() else np.zeros((0, 3), dtype=np.float32)
