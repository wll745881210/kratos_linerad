...
def neufeld_peak(a_tau0):
    """Neufeld (1990) peak: |x_p| = 0.881 * (a*tau0)**(1/3), tau0 = mean depth."""
...
def generate_kratos_inputs(tau0_fid, a_voigt, n_radiation, out_dir, tag,
                           ...
                           t_lim=1800.0, seed=42, ph_mode=1):
    """Generate Kratos field/photon/par files for a target mean depth tau0_fid.
...
def run_kratos_one(tau0_fid, a_voigt, n_radiation, out_dir, tag, ph_mode=1):
    ...
    print(f"    n_esc={len(x_freq)}, x_peak={x_peak:.3f},"
          f" med|x|={np.median(abs_x):.3f}, n_step={n_step}")
    return {
        'x_freq': x_freq, 'x_peak': x_peak,
        'med_x': np.median(abs_x), 'n_esc': len(x_freq),
    }
...
def run_python_one(tau0_fid, a_voigt, n_radiation):
    ...
    """
    tau0_mcrt = 2.0 * tau0_fid / np.sqrt(np.pi)
    print(f"  Python: tau0_fid={tau0_fid:.1f}, tau0_mcrt={tau0_mcrt:.1f},"
          f" a={a_voigt}, n={n_radiation}")
    result = mcrt_slab(
...
def main():
    ...
    p.add_argument('--a', dest='a_voigt', type=float, default=0.149,
                   help='Voigt a parameter (default 0.149, T~10K Lyα, '
                        'used by Verhamme 2006 and other Lyα papers)')
    p.add_argument('--n', dest='n_radiation', type=int, default=10000)
    p.add_argument('--ph-mode-list', type=int, nargs='+', default=[1, 2],
                   help='Kratos ph_mode values to test (1=R_IIA v1 global, '
                        '2=R_IIA coarse const-mem). Pass "1 2" to compare.')
    ...
    for tau0_fid in args.tau0_fid_list:
        ...
        print(f"\n=== tau0={tau0_fid:.0f} (mean-depth), a={args.a_voigt},"
              f" a*tau0={a_tau0:.0f}, (a*tau0)^(1/3)={at13:.2f},"
        ...
        if not args.no_kratos:
            for pm in ph_modes:
                ...
                kres = run_kratos_one(tau0_fid, args.a_voigt,
                                      args.n_radiation, WORKDIR, tag,
                                      ph_mode=pm)
                if kres:
                    entry[f'kratos_peak_pm{pm}'] = kres['x_peak']
                    entry[f'kratos_med_pm{pm}'] = kres['med_x']
                    entry[f'kratos_x_pm{pm}'] = kres['x_freq']

        if not args.no_python:
            pres = run_python_one(tau0_fid, args.a_voigt, args.n_radiation)
            if pres:
    ...
    pm_styles = {
        ...
        2: ('b', '^', '--', 'ph_mode=2 (R_IIA coarse, const-mem, du=0.048)'),
    }

    def kratos_pm_keys(r):
        return sorted(k for k in r if k.startswith('kratos_peak_pm'))

    # ── Plot 1: |x_peak| vs a*tau0 ───────────────────────────────
    fig, ax = plt.subplots(1, 1, figsize=(8, 6))
    at_arr = np.array([r['a_tau0'] for r in results])
    ...
    first = results[0]
    for pm in ph_modes:
        key = f'kratos_peak_pm{pm}'
        if key in first:
            c, m, ls, lbl = pm_styles.get(pm, ('g', 'o', '-', f'pm{pm}'))
            kp = np.array([r.get(key, np.nan) for r in results])[sort_idx]
            ax.plot(at_arr[sort_idx], kp, color=c, marker=m, linestyle=ls,
                    markersize=8, linewidth=1.5, label=f'Kratos ({lbl})')
    if 'python_peak' in first:
        pp = np.array([r.get('python_peak', np.nan) for r in results])[sort_idx]
        ax.plot(at_arr[sort_idx], pp, 'g^--', markersize=8, linewidth=1.5,
    ...
    for pm in ph_modes:
        ...
        if key in first:
            c, m, ls, lbl = pm_styles.get(pm, ('g', 'o', '-', f'pm{pm}'))
            kr = np.array([r.get(key, np.nan) / r['neufeld_peak']
                           for r in results])[sort_idx]
            ax.plot(at_arr[sort_idx], kr, color=c, marker=m, linestyle=ls,
                    markersize=8, label=f'Kratos ({lbl}) / Neufeld')
    if 'python_peak' in first:
    ...
    for idx, r in enumerate(results):
        ...
        has_legend = False
        for pm in ph_modes:
            xkey = f'kratos_x_pm{pm}'
            if xkey in r:
                c, m, ls, lbl = pm_styles.get(pm, ('g', 'o', '-', f'pm{pm}'))
                h, _ = np.histogram(r[xkey], bins=bins, density=True)
                ax.plot(bc, h, color=c, linestyle=ls, linewidth=1.5,
                        label=f'Kratos ph_mode={pm}')
                has_legend = True
        if 'python_x' in r:
            h, _ = np.histogram(r['python_x'], bins=bins, density=True)
            ax.plot(bc, h, 'g--', linewidth=1.5, label='Python')
            has_legend = True

        J = neufeld_J(bc, a_tau0)
        norm = np.trapezoid(J, bc)
        if float(norm) > 0:
            J /= norm
        ...
        for s in (1, -1):
            ax.axvline(s * xp, color='gray', linestyle=':', linewidth=0.8)
        ...
        ax.set_title(
            f'$\\tau_0={r["tau0"]:.0f}$,'
            f' $(a\\tau_0)^{{1/3}}={at13:.1f}$', fontsize=11)
        if has_legend:
    ...
    fig.suptitle(
        'Emergent spectra: Kratos ph_mode comparison vs Neufeld analytic'
        f' ($a={args.a_voigt}$, isotropic midplane)', fontsize=13, y=1.01)
    ...
    # ── Summary table ────────────────────────────────────────────
    pm_cols = ''.join(f' {f"K(pm{pm})":>8} {"med":>7}' for pm in ph_modes)
    header = (f"{'tau0':>7} {'a*tau0':>8} {'(at)^(1/3)':>10} {'Neufeld':>8}"
              + pm_cols
              + (f" {'Python':>8}" if not args.no_python else '')
              + (f" {'P/N':>6}" if not args.no_python else ''))
    print(f"\n{'='*len(header)}")
    print(header)
    print(f"{'-'*len(header)}")
    for r in results:
        np_ = r['neufeld_peak']
        line = (f"{r['tau0']:7.0f} {r['a_tau0']:8.0f}"
                f" {r['at13']:10.1f} {np_:8.3f}")
        for pm in ph_modes:
            kp = r.get(f'kratos_peak_pm{pm}', np.nan)
            km = r.get(f'kratos_med_pm{pm}', np.nan)
            line += f" {kp:8.3f} {km:7.3f}"
        if not args.no_python:
            pp = r.get('python_peak', np.nan)
            line += f" {pp:8.3f} {pp/np_:6.3f}"
        print(line)
    print(f"{'='*len(header)}")
