...
////////////////////////////////////////////////////////////
// Utility function for Voigt sampling -- not used now
...
float_t voigt_H( const float_t & a, const float_t & u )
{
    ...
    const float_t u0   = sqrtf( logf( core > 1e-38f ? wing
                                    / core + 1 : 1e36f ) );
...
< crtp::helper<line_rt_photon_t, derived_T > >
{
    ...
    float_t     amplitude;
    float_t    tau_remain;
    // Initial proper weight (set at generation; used by
    // the proper-weight culling in proc_step).
    float_t      proper_0;
    // Position of the last scattering event (initialised
    // to the generation position; updated in scat()).
      ...
      const itg_T &     itg )
    {
        ...
        // Record the last-scattering position BEFORE moving
        // (the scattering happens at the current position).
        for( int a = 0; a < 3; ++ a )
            x_last_scat[ a ] = x[ a ];
        ...
        const auto mu  = 2 * device::rand_dev(  ) - 1;
        const auto smu = sqrtf( 1 - mu * mu );
        const auto phi = 6.28318531f * device::rand_dev(  );
        dir[ 0 ] = smu * cosf( phi );
        dir[ 1 ] = smu * sinf( phi );
        dir[ 2 ] =  mu ;
        ...
        // the new vel is first set to the gas-frame outgoing
        // frequency offset dv_new, then converted to the
        // stored convention  vel = dv_new - vel_obs_new
        // (where vel_obs_new = dir_new . v_bulk) so that
        // later  dv = vel + vel_obs  recovers dv_new.
        if( itg.ph_mode == 0 )
            vel = r * cosf( 6.283185307f * u2 )
                * b_sca / sqrt_2;
        else
        {
            const float_t x_freq = vel_lab_old / b_sca;
    ...
    {             
        ...
        {   // Rescale dl_a; also used in absorption.
        ...
        const auto flx = proper * dsi *
           ( dtau_a > 1e-3f ? ( 1 - e_mtau ) / dtau_a : 1 );
     ...
     const bmp_T &bmap, const itg_T &itg)
    {
        const auto proc_flag
            = super_t::proc_step( dl, g_l, bmap, itg );
        if( step <= 0 )
        {
            dest. i_rank = -1;
            return false ;
        ...
        // weight has decayed below proper_min_frac * proper_0
        // (set on intg_t, not on the photon, to save space).
        // proper_0 is set at generation (gen.h).  Disabled
        // when proper_min_frac <= 0.
