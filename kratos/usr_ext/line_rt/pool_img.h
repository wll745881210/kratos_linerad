...
{
////////////////////////////////////////////////////////////
//  Imaging photon pool: writes the per-pixel, per-channel
//  image cube.  Each "particle" is one camera ray (one
//  pixel); its I_chan[0..n_chan-1] array holds the emergent
//  intensity per velocity channel.
// Output keys (all per escaped ray):
...
struct pol_img_t : particle::pool_t< line_img_t<  > >
{
    ...
    __host__ pol_img_t(  ) : base_t(  ), n_chan_img( 0 ),
