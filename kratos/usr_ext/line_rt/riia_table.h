...
////////////////////////////////////////////////////////////
// riia_table_t: standalone R_IIA redistribution table.
//
// Owns ALL R_IIA + USampler data:
//   - 3-D table R(Δ; |x_pp|, g), io-fastest layout
//   - USampler log(CDF) + x grid (device-resident)
//   - GPU construction kernel (float_t, reads d_cdf + d_xg)
//
// Device methods:
//   operator()  — table indexing (io-fastest)
//   _invcdf     — USampler inverse CDF lookup
//   sample_upar — USampler sampling (for scattering photon.h)
//   lookup      — R_IIA trilinear lookup (for s_cam accumulation)
//
// Host methods:
//   build       — full construction (USampler CDF on host + R_IIA on GPU)
//   free        — device memory cleanup
//
// intg_t holds a single riia_table_t riia; member and delegates:
//   riia.sample_upar(), riia.lookup(), riia.build(), riia.free().
//
// Future: _build_usampler() can be replaced by a GPU kernel;
//   _build_riia_gpu() stays unchanged (reads d_cdf + d_xg
//   from device, never touches host arrays).
////////////////////////////////////////////////////////////
...
{
    ////// Device pointers //////
    float_t *  dat;               // R_IIA table (n_xo, n_xp, n_g)
    ...
    __host__ riia_table_t()
      : dat(nullptr), d_cdf(nullptr), d_xg(nullptr),
        use_const_mem(false),
        n_xo(200), n_xp(200), n_g(40),
        xo_max(10.f), xp_max(120.f),
        dxo(0), dxp(0), dg(0),
        n_u(251), n_xg(40),
        du(0.048f), u_max(6.f),
        a_voigt(0.f)
    ...
    ////// Indexing (device, io-fastest) //////
    // io-fastest layout: io + n_xo*(jp + n_xp*ig)
    // (better GPU coalescing than the old io-slowest)
    __device__ __forceinline__
    float_t & operator() ( int io, int jp, int ig ) const
    { return dat[ io + n_xo * ( jp + n_xp * ig ) ]; }

    //////////////////////////////////////////////////
    // USampler inverse CDF (device, private).
    // Binary search on log(CDF) row for log(r),
    ...
    ( const float_t * log_cdf, float_t r ) const
    {
        ...
        if( k > n_u - 2 ) k = n_u - 2;
        if( k < 1 )       k = 1;
        float_t denom = log_cdf[ k ] - log_cdf[ k - 1 ];
        if( denom < 1e-35f ) denom = 1e-35f;
        const float_t frac = ( log_r - log_cdf[ k - 1 ] ) / denom;
        float_t u = ( -u_max + du * float_t( k - 1 ) ) + frac * du;
        if( u >  u_max ) u =  u_max;
        if( u < -u_max ) u = -u_max;
        return u;
    }
    ...
    //
    // For |g| > (1 - dg) — the last 2 grid points on
    ...
    // (g=0.949, σ≈0.22) and a delta spike (g=1.0, σ≈0)
    // cannot capture the qualitative shape change, so
    // use the analytic form R = Gauss(Δ; σ=sin_g/√π).
    __device__ __forceinline__
    float_t lookup
    ( float_t x_out, float_t x_pp, float_t g ) const
    {
        ...
        if( ixp > n_xp - 2 ) ixp = n_xp - 2;
        if( ixp < 0 )         ixp = 0;
        float_t fxp = ( ax_pp - ixp * dxp ) / dxp;
        if( fxp > 1.f ) fxp = 1.f;
        if( fxp < 0.f ) fxp = 0.f;

        int ixo = int( ( t_delta + xo_max ) / dxo );
        if( ixo > n_xo - 2 ) ixo = n_xo - 2;
        if( ixo < 0 )         ixo = 0;
        float_t fxo = ( t_delta + xo_max - ixo * dxo ) / dxo;
        if( fxo > 1.f ) fxo = 1.f;
        if( fxo < 0.f ) fxo = 0.f;
        ...
        if( ig < 0 )       ig = 0;
        float_t fg = ( g + 1.f - ig * dg ) / dg;
        if( fg > 1.f ) fg = 1.f;
        if( fg < 0.f ) fg = 0.f;

        const auto c00 = operator()(ixo,   ixp,   ig  ) * ( 1.f - fg )
                       + operator()(ixo,   ixp,   ig+1) * fg;
        const auto c01 = operator()(ixo,   ixp+1, ig  ) * ( 1.f - fg )
                       + operator()(ixo,   ixp+1, ig+1) * fg;
        const auto c10 = operator()(ixo+1, ixp,   ig  ) * ( 1.f - fg )
                       + operator()(ixo+1, ixp,   ig+1) * fg;
        const auto c11 = operator()(ixo+1, ixp+1, ig  ) * ( 1.f - fg )
                       + operator()(ixo+1, ixp+1, ig+1) * fg;
    ...
    __host__ void build        ( device::base_t & dev, float_t a_voigt );
    __host__ void free         ( device::base_t & dev );
    __host__ void _build_usampler ( device::base_t & dev, float_t a_voigt );
...
// the same USampler CDF row → broadcast access (L2 cache).
////////////////////////////////////////////////////////////

static __global__ void build_riia_gpu_kernel ( riia_table_t riia )
{
    int io = blockIdx.x * blockDim.x + threadIdx.x;
    int jp = blockIdx.y;
    int ig = blockIdx.z;
    if( io >= riia.n_xo || jp >= riia.n_xp || ig >= riia.n_g )
        return;
    ...
    if( jxg < 0 )               jxg = 0;
    if( jxg >= riia.n_xg - 1 )  jxg = riia.n_xg - 2;
...
//
// P(u|x) ~ exp(-u²) / (a² + (x-u)²).
// log(CDF) is stored for smooth interpolation in the
// tails (where the CDF is nearly flat).
// use_const_mem: global (freed in free()) or const
// (system-managed pool, never freed).
////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////
// _build_riia_gpu: allocate R_IIA table on device + launch
// GPU construction kernel.  Reads d_cdf + d_xg (already
// on device from _build_usampler).  All float_t.
////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////
// free: release device memory.
//   dat: always freed (always global memory).
//   d_cdf, d_xg: freed only if !use_const_mem (global).
//   Const-mem pointers are NOT freed (system-managed pool).
////////////////////////////////////////////////////////////
