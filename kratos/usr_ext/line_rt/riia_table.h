...
struct riia_table_t
{
    ...
    //  io-fastest layout:
    //  io + n_xo * ( jp + n_xp * ig )
    //  (better GPU coalescing than the
    //  old io-slowest)
    ...
    //  Sample u_par for a given xa
    //  (|xa| with sign restored):
    //  binary search on the xg grid,
    //  then interp between adjacent
    //  rows of the log(CDF) table.
    __device__ __forceinline__
    ...
    //  R(x_out; x_pp, g) [x-space,
    //  integral R dx = 1]
    //  Uses the 3-D table built by the
    //  GPU kernel.  The table is
    //  parametrised in delta = x_out -
    //  x_pp (symmetry:
    //  R(delta; -x_pp, g) =
    //  R(-delta; x_pp, g)), so the
    //  device lookup computes
    //  t_delta = delta * sgn.
    //
    //  For |x_pp| >= xp_max the USampler
    //  CDF has converged to the
    //  asymptotic form pdf_inf ~
    //  exp(-u^2), giving the analytic
    //  kernel:
    //    R_inf(delta; g) =
    //      exp(-delta^2 /
    //        ((g-1)^2 + sin^2_g))
    //      / (sqrt(pi) *
    //         sqrt((g-1)^2+sin^2_g))
    //  which is used directly (no table
    //  lookup needed).
    //
    //  For |g| > (1 - dg) -- the last 2
    //  grid points on each side -- the
    //  R_IIA kernel collapses to a
    //  narrow Gaussian (delta-function
    //  limit as g -> +/-1).
    //  Trilinear interpolation between
    //  a broad Gaussian (g=0.949,
    //  sigma~0.22) and a delta spike
    //  (g=1.0, sigma~0) cannot capture
    //  the qualitative shape change, so
    //  use the analytic form
    //  R = Gauss(delta; sigma=sin_g/
    //             sqrt(pi)).
    __device__ __forceinline__
    float_t lookup
    ( float_t x_out, float_t x_pp,
      float_t g ) const
    {
    ...
    ( device::base_t & dev,
      float_t a_voigt );
    __host__ void _build_riia_gpu
    ( device::base_t & dev );
};

////////////////////////////////////////////////////////////
//  GPU kernel: build USampler log(CDF)
//  on device.  Each thread computes one
//  row (one xg value) of the CDF:
//  P(u|x) ~ exp(-u^2) /
//  (a^2 + (x-u)^2), cumulative sum,
//  normalize, log.  All float_t
//  (single precision).
...
////////////////////////////////////////////////////////////
//  GPU kernel: build R_IIA table from
//  device-resident USampler log(CDF) +
//  x grid.  Uses float_t (single
//  precision) throughout.  Each thread
//  computes one (io, jp, ig) entry.
//
//  Grid: block (64, 1, 1), grid
//  (ceil(n_xo/64), n_xp, n_g).
//  Threads in the same warp (same jp,
//  different io) read the same USampler
//  CDF row -> broadcast access
//  (L2 cache).
////////////////////////////////////////////////////////////
...
( riia_table_t riia )
{
    ...
    //  Read log(CDF) row, convert via
    //  expf to raw CDF, compute pdf on
    //  the fly, accumulate R.
    const float_t * row
        = riia.d_cdf + jxg * riia.n_u;
...
////////////////////////////////////////////////////////////
//  _build_usampler: USampler CDF
//  construction on HOST.  (Future:
//  replace with a GPU kernel --
//  _build_riia_gpu stays unchanged, it
//  reads d_cdf + d_xg from device.)
//
//  P(u|x) ~ exp(-u^2) /
//  (a^2 + (x-u)^2).
//  log(CDF) is stored for smooth
//  interpolation in the tails (where
//  the CDF is nearly flat).
//  use_const_mem: global (freed in
//  free()) or const (system-managed
//  pool, never freed).
////////////////////////////////////////////////////////////
...
( device::base_t & dev, float_t a_voigt )
{
    ...
    //  Allocate d_cdf + d_xg in
    //  global memory.
    const size_t n_total
        = size_t( n_u ) * n_xg;
    d_cdf = dev.malloc_device< float_t >
        ( n_total );
    d_xg = dev.malloc_device< float_t >
        ( n_xg );
    dev.cp( d_xg, h_xg, n_xg );

    //  Launch GPU kernel to build CDF
    //  (40 threads, each computes one
    //  row of 251 values).
    dim3 block( n_xg, 1, 1 );
    dim3 grid( 1, 1, 1 );
    ...
    //  Option B: copy USampler CDF +
    //  xg to const memory for
    //  broadcast-cache reads in the
    //  MCRT hot path.  R_IIA table
    //  (dat) stays in global
    //  (too large).
    if( use_const_mem )
    {
        float_t * d_cdf_c
            = dev.malloc_const< float_t >
              ( n_total );
        float_t * d_xg_c
            = dev.malloc_const< float_t >
              ( n_xg );
...
( device::base_t & dev )
{
    ...
    const size_t n_tab
        = size_t( n_xo ) * n_xp * n_g;
    dat = dev.malloc_device< float_t >
        ( n_tab );

    dim3 block( 64, 1, 1 );
    dim3 grid( ( n_xo + 63 ) / 64,
               n_xp, n_g );

    dev.launch( build_riia_gpu_kernel,
        grid, block, 0,
        ( const void * )0, * this );
    dev.sync_all_streams(  );
}
...
//    dat: always freed (always global
//    memory).
//    d_cdf, d_xg: freed only if
//    !use_const_mem (global).
//    Const-mem pointers are NOT freed
//    (system-managed pool).
////////////////////////////////////////////////////////////
