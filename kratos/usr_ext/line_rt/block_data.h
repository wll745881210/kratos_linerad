...
struct rad_t
{
    ...
    ////////// Functions //////////
    __host__ rad_t(  ) : ray_output( false ), ray_id ( -1 ),
                         imaging( false ), n_chan( 0 ) {  };
...
struct block_data_t : mesh::block::base_data_t
{
    ...
    __host__ virtual void setup( const f_new_t & f_n )
    {
        ...
        rad.mfp_i_sca_0.init( f_n, geo.n_ceff, n_gh, 1 );
        rad.mfp_i_abs_0.init( f_n, geo.n_ceff, n_gh, 1 );
        rad.      b_sca.init( f_n, geo.n_ceff, n_gh, 1 );
        rad.        vel.init( f_n, geo.n_ceff, n_gh, 3 );
        rad.    flx.init( f_n, geo.n_ceff, n_gh, n_fld );
        rad.excitation_flux.init
            ( f_n, geo.n_ceff, n_gh, n_fld );

        rad.emiss.init( f_n, geo.n_ceff, n_gh, 1 );

        rad.j_cam.init( f_n, geo.n_ceff, n_gh,
    ...
    ( const f_cp_t & f_cp, mesh::block::base_data_t & tgt_ )
    {   // Intentionally blank -- init on the GPU side
        return;
    };
    ...
    {
        auto & tgt = dynamic_cast< block_data_t & >( tgt_ );
        tgt.output = this->output;
        rad.mfp_i_sca_0.cp_to( tgt.rad.mfp_i_sca_0, f_cp );
        rad.mfp_i_abs_0.cp_to( tgt.rad.mfp_i_abs_0, f_cp );
        rad.excitation_flux.cp_to
             ( tgt.rad.excitation_flux, f_cp );
        rad.b_sca.cp_to( tgt.rad.b_sca, f_cp );
        rad.  flx.cp_to( tgt.rad.  flx, f_cp );
    ...
    {
        rad.b_sca.read( f_r, rad.prefix(  ) + "b_sca_" );
        rad.  flx.read( f_r, rad.prefix(  ) +   "flx_" );
        rad.  vel.read( f_r, rad.prefix(  ) +   "vel_" );
        rad.emiss.read( f_r, rad.prefix(  ) + "emiss_" );
        rad.mfp_i_sca_0.read
                ( f_r, rad.prefix(  ) + "mfp_i_sca_0_" );
        rad.mfp_i_abs_0.read
                ( f_r, rad.prefix(  ) + "mfp_i_abs_0_" );
        return rad.excitation_flux.read
...
struct prx_t: particle::radiation::proxy_t< block_data_t >
{
    ////////// Type and data //////////
    ...
    template< class bdt_T > __forceinline__ __host__
