...
( const  input & args, particle::base_t & mod )
{
    ...
    output = rimg.enabled;
    // Read n_chan directly from par (the integrator may not
    // have been initialised yet at this point in the module
    // init sequence).
...
( const mesh::f_cp_t & f_cp, const mesh::f_write_t & f_w )
{
    ...
    {
        // Off-image / failed rays carry i_rank == -2; skip them.
