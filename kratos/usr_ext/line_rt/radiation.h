...
public:                         // Type
    struct ini_t
    {
        ...
        // Imaging configuration (read from par, mirrored
        // into rad_t via block_yield so the device proxy
        // sees them).
        bool imaging = false;
        int   n_chan =       0;
        ...
        // Must be called after p_dev is available,
        // in init(  ), NOT in read(  ).
...
public:
    virtual void read( const input & args ) override
    {
        ...
        // so block_data_t::setup() allocates j_cam with the
