...
{
    // Server-worker work counter (device).  Reset to 0 in
    // pre_proc; each worker atomically fetches the next
    // photon index via load_next() until n_par is reached.
      ...
      const bool & keep_data = true ) override
    {
        ...
        {
            work_counter = mod.p_dev->malloc_device< int_t >( 1 );
            mod.p_dev->f_mset( work_counter, 0,
                               sizeof( int_t ), mod.stream );
        }
        return base_t::set_mem( mod, n_par, safe, keep_data );
    };

    __host__ virtual void finalize( particle::base_t & mod ) override
    {
        if( work_counter != nullptr )
        {
    ...
    __host__ virtual void pre_proc( particle::base_t & mod ) override
    {
        if( work_counter != nullptr )
            mod.p_dev->f_mset( work_counter, 0,
                               sizeof( int_t ), mod.stream );
    ...
    {
        const auto i = utils::atomic_inc( work_counter ) + offset;
        flag   = ( i < n_par );
        if( flag )
            par.load( ( * this )[ i ] );
        return i;
      ...
      const mesh::f_write_t & f_w )
    {
        ...
        for( size_t i = 0; i < n_par; ++ i )
        {
            ...
            for( int a = 0; a < 3; ++ a )
            {
                ...
                xls_h[ 3 * j + a ] = par_h[ i ].x_last_scat[ a ];
            }
