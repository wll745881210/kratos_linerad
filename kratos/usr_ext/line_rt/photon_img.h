                  ...
                  < crtp::helper< line_img_t, derived_T > >
{
      ...
      const itg_T     & itg )
    {
        ...
        //  dir_cam . v_bulk : the gas bulk projected onto
        //  the camera LOS (dir_cam points INTO the domain).
        float_t vobs_cam( 0 );
        for( int a = 0; a < 3; ++ a )
    ...
    //  Per-pixel entry: set up the camera ray and run the
    //  ray-march loop (mirrors polarized_img_t::proc).
    ...
    int proc( const map_T & bmap, const itg_T & itg )
    {
        ...
        //  Ray direction = camera LOS (into the domain).
        //  In the camera frame the LOS is +z, which q_cam
        //  maps to dir_cam in the domain frame.
        for( int a = 0; a < 3; ++ a )
            dir[ a ] = itg.dir_cam[ a ];
