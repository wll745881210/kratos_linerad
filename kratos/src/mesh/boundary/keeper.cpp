...
void keeper_t::finalize(  )
{   // Release the per-keeper split communicator while the
    // device context is still alive. This must run before
    // mesh_t::~mesh_t calls p_dev->finalize( ), otherwise
    // device_t::unpin inside ~mpi_t -> local_t::free fails.
    bnd_comm.clear(  );
    return p_com->finalize(  );
}   // Triggers ~mpi_t -> finalize -> unpin
