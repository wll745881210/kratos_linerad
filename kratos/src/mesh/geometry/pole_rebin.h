...
struct rebin_t
{
    ...
    __host__ virtual void read( const input & args )
    {
        ...
        n_th_lim    = args.get< int >
            ( "mesh", "pole_rebin_threadlim", 32 );
    ...
    {        
        type::idx_t i;
        i[ 0 ] = threadIdx.x + blockIdx.x * blockDim.x;
        if( i[ 0 ] >= b_d.geo.n_ceff[ 0 ] )
            return;
        
        if( side == 0 )
            i[ 1 ] = blockIdx.y;
        else
            i[ 1 ] = b_d.geo.n_ceff[ 1 ] - 1 - blockIdx.y;
                   ...
                   const device::base_t & dev ) const
    {
        ...
        using f_t = typename type::prim_type_t
                  < decltype( u ) >::f_t;
        const int n_bl_y = utils::min
            ( rebin_lvl, b_d.geo.n_ceff[ 1 ] );
        const int n_th   = utils::min
            ( n_th_lim,  b_d.geo.n_ceff[ 0 ] );
        const int n_bl_x =
            ( b_d.geo.n_ceff[ 0 ] + n_th - 1 ) / n_th;
        const int n_sh = n_var * sizeof( f_t ) * n_th;
        const auto rsc = std::make_tuple
            ( dim3( n_bl_x, n_bl_y ), dim3( n_th ), n_sh );
