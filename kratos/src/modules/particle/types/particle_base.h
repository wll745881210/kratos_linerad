...
struct par_base_t
{
    ...
    __device__ bool regulate( const map_T & bmap )
    {
        ...
        for( int a = 0; a < x.size(  ); ++ a )
        {
            ...
            {   // "<=" is needed when wrapping
                x[ a ] += bmap.l_box[ a ];
                res     = true;
            }
            else if( x[ a ] >= bmap.xlim [ 1 ][ a ] )
            {   // Must be "else if" for low precision
                x[ a ] -= bmap.l_box[ a ];
                res     = true;
            }
            if( x[ a ] > bmap.xlim[ 1 ][ a ] )
                x[ a ] = bmap.xlim[ 1 ][ a ] ;
            if( x[ a ] < bmap.xlim[ 0 ][ a ] )
                x[ a ] = bmap.xlim[ 0 ][ a ] ;
        }
...
struct geo_loc_t
{
    ...
    template< class par_T > __device__ __forceinline__    
    void fix( const mesh::geo_t & geo, par_T & par )
    {
        ...
        }// Previously OpenCode + DS put this "fix"
         // (originally wrong!) unconditionally, breaking
         // the particle transport as a whole (increment of
         // par.i has already been done in particle's
         // proc_geo). However, the thoughts behind this
         // incorrect fix are still valuable: particles
         // crossing vertices need a place to account for.
        return; // See particle_base.h : 46
    };
