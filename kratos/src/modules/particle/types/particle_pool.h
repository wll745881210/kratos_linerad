...
template  < class     par_T , class int_T = unsigned int,
...
public:
      ...
      const bool & keep_data = true )
    {
        ...
        if( capacity >= n_par && n_par >= capacity / 2 )
        {
            this-> n_par = n_par ;
            return false ;
        }
        capacity = utils::max
            ( n_par + n_par * ( safe - 1 ), capacity / 4 );
...
// #warning "TODO: Blocked pool for size increase!"
    ...
    __host__ int_t n_par_eff(  ) const
    {
        return n_par - offset;
    };
    ...
    __host__ virtual
    void post_proc( particle::base_t & mod ) {  };
    ...
    ( const int_T & n_keep, const int_T & n_mv ) const
    {
        ...
        if( i >= n_mv )
            return;
        auto & src = get_self(   )[ n_keep + i ];
        if( src.dest.todo != to_keep )
            return;
                
        int_T  j ( 0 );
        while( j < n_mv )
        {
            j = utils::atomic_inc( i_mv + 1 );
            const auto i_tgt = map_pars [ j ];
            if( i_tgt < n_keep )
            {
