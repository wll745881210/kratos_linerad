...
struct const_list_t
{
    ...
    __host__ void free( device::base_t & f_dev )
    {
        if( ! on_const )
            f_dev.free( data );
        return;
    ...
    __host__ void regularize
    ( device::base_t & f_dev, const bool & on_const = true )
    {
        ...
        void *  p_c;
        if( on_const )
            p_c = f_dev.f_malloc_const( size_byte );
        else
            p_c = f_dev.f_malloc      ( size_byte );
