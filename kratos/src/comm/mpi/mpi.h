...
protected:                      // Data
    ...
    bool            finalized;  // Idempotency 4 finalize( )
public:                         // Function
     mpi_t(  );
    ~mpi_t(  );
...
protected:                      // Data
    MPI_Comm                   l_comm;
    int                       nl_rank;
    std::map< int, int >   lrank_of_rank;
    bool               local_optimize;
    bool          local_window_locked;
    bool          local_window_pinned;
    void              * local_dat0;
    size_t          local_capacity;
    MPI_Win           local_window;
    std::vector< size_t > local_capacity_all;
    std::vector< local_t > local_send;
    std::vector< local_t > local_recv;    
protected:                      // Functions
    template< class fun_T >
    void    map_local ( const fun_T & f );
    virtual int  lrank_of( const int &  );
