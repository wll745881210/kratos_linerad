...
def iterate( source_photons, species, fields_init, mesh, \
    ...
    for cycle in range( n_cycles ):
        ...
        # the emission seed (emiss/(mfp_i_sca*sqrt(pi)*b))
        # carried by s_cam lives in the same scaled-proper
        ...
        # scattering s_cam is in scaled units.  The emissivity
            ...
            if 'l' in img and 'i2d' in img:
                ...
                if n_chan > 0 and \
                   l_flat.size == n_pix * n_chan:
                    ...
                    # Undo proper scaling: s_cam is a sum of
                    # scaled photon propers (scattering part)
                    # plus an emission seed built from the
